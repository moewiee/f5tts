...
def infer_batch(ref_audio, ref_text, gen_text_batches, exp_name, progress=gr.Progress()):
    if exp_name == "v1":
        ema_model = v1_model
    elif exp_name == "v0":
        ema_model = v0_model
...
def infer(ref_audio_orig, gen_text, exp_name):
    ...
    return infer_batch((audio, sr), ref_text, gen_text_batches, exp_name)
    ...
    with gr.Row():
        with gr.Column():
            ...
            model_choice = gr.Radio(
                choices=["v1", "v0"], label="Model choice: v1 trained with more data but less steps than v0", value="v1"
            )
        with gr.Column():
